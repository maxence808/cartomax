...
def trouver_nomfic_pdf_par_api_gpu(code_insee: str, details_api: dict) -> str:
    ...
    details_api["recherche_document"] = trace_document
...
def est_pdf_reglement(nom: str) -> bool:
    ...
    if not nom_minuscule.endswith(".pdf"):
        return False
    mots_reglement = ("reglement", "regl", "reglem", "piece_ecrite", "ecrit")
    mots_a_eviter = ("annexe", "plan", "zonage", "prescription", "servitude", "oap", "padd")
    return any(mot in nom_minuscule for mot in mots_reglement) and not any(
        mot in nom_minuscule for mot in mots_a_eviter
    )
